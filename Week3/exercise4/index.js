...
async function findEpisodesExercises(client) {
  ...
  console.log(
    `The title of episode 2 in season 2 is ${titleResult.title}`
  );
  ...
  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${seasonResult.episode}} `
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]

  const cliffResult = await client
                      .db("databaseWeek3")
                      .collection("bob_ross_episodes")
                      .find({elements: "CLIFF"})
                      .toArray()
  const titleWithCliff = cliffResult.map(episode => episode.title)
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${titleWithCliff}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
  const cliffAndLightHouseResult = await client
                      .db("databaseWeek3")
                      .collection("bob_ross_episodes")
  ...
  const titleWithCliffAndLightHouse = cliffAndLightHouseResult.map(episode => episode.title)                    
  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${titleWithCliffAndLightHouse}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const result = await client
                      .db("databaseWeek3")
                      .collection("bob_ross_episodes")
                      .updateOne({title:"BLUE RIDGE FALLERS"},
  ...
  const bushResult = await client
                      .db("databaseWeek3")
                      .collection("bob_ross_episodes")
                      .updateMany({}, {$set: {"elements.$[element]":"BUSH"}}, { arrayFilters: [{ element: "BUSHES" }]})
  console.log(
